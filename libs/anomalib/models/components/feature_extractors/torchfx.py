...
class TorchFXFeatureExtractor(nn.Module):
    ...
    Args:
        ...
        weights (str | WeightsEnum | None): Weights enum to use for the model. Torchvision custom_models require
            ``WeightsEnum``. These enums are defined in ``torchvision.custom_models.<model>``. You can pass the weights
            path for custom custom_models.
    ...
    Example:
        With torchvision custom_models:

            >>> import torch
            >>> from anomalib import TorchFXFeatureExtractor
            >>> from torchvision.custom_models.efficientnet import EfficientNet_B5_Weights
    ...
    def initialize_feature_extractor(
        ...
        if isinstance(backbone, nn.Module):
            ...
            if isinstance(weights, WeightsEnum):  # torchvision custom_models
    ...
    def _get_backbone_class(backbone: str) -> Callable[..., nn.Module]:
        ...
        Example:
            ...
            <function torchvision.custom_models.efficientnet.efficientnet_b5(
                *,
                weights: torchvision.custom_models.efficientnet.EfficientNet_B5_Weights | NoneType = None,
                progress: bool = True,
                **kwargs: Any
                ) -> torchvision.custom_models.efficientnet.EfficientNet>
        ...
        try:
            if len(backbone.split(".")) > 1:
                ...
                models = importlib.import_module("torchvision.custom_models")
                backbone_class = getattr(models, backbone)
        except ModuleNotFoundError as exception:
            raise ModuleNotFoundError(
                f"Backbone {backbone} not found in torchvision.custom_models nor in {backbone} module."
            ) from exception
