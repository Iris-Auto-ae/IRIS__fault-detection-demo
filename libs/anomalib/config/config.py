...
def update_datasets_config(config: DictConfig | ListConfig) -> DictConfig | ListConfig:
    ...
    if "clip_length_in_frames" in config.dataset.keys() and config.dataset.clip_length_in_frames > 1:
        warn(
            "Anomalib's custom_models and visualizer are currently not compatible with video datasets with a clip length > 1. "
...
def get_configurable_parameters(
    ...
    if config_path is None:
        config_path = Path(f"src/anomalib/custom_models/{model_name}/{config_filename}.{config_file_extension}")
